...
-- Optional contact info a member can choose to share, plus a flag for
-- whether they're OK being reached that way. Safe to re-run.
...
alter table member_profiles add column if not exists ok_to_contact boolean not null default false;
